...
class VaultImagePicker extends FuzzySuggestModal {
    constructor(app, folder, onChoose) {
        ...
        this.setPlaceholder(
            this.folder
                ? `Search image in "${this.folder}"...`
                : "Search image in vault...",
        );
    }
    getItems() {
        return this.app.vault.getFiles().filter((f) => {
            if (
                !/\.(png|jpe?g|gif|bmp|webp|svg)$/i.test(
                    f.extension ? "." + f.extension : f.path,
                )
            )
                return false;
...
function smoothPoints(rawPts, factor = 10) {
    ...
    if (
        Math.hypot(
            last.x - out[out.length - 1].x,
            last.y - out[out.length - 1].y,
        ) > 0.1
    )
        out.push(last);
    return out;
...
class InsertPageModal extends Modal {
    ...
    onOpen() {
        ...
        new Setting(el)
            .setName("Reference page")
            .setDesc(`Current: ${this.cur} / ${this.total}`)
            .addText((t) => {
                t.setValue(String(this.opts.targetPage)).onChange((v) => {
                    const n = parseInt(v);
                    if (!isNaN(n) && n >= 1 && n <= this.total)
                        this.opts.targetPage = n;
                });
                t.inputEl.type = "number";
                t.inputEl.min = "1";
                t.inputEl.max = String(this.total);
        ...
        new Setting(el).setName("Position").addDropdown((d) =>
            d
                ...
                .setValue(this.opts.position)
                .onChange((v) => (this.opts.position = v)),
        );
        new Setting(el).setName("Page type").addDropdown((d) => {
            PAGE_TYPES.forEach((p) => d.addOption(p.id, p.label));
            d.setValue(this.opts.pageType).onChange(
                (v) => (this.opts.pageType = v),
            );
        });
        const row = el.createDiv({
            attr: {
                style: "display:flex;gap:10px;margin-top:18px;justify-content:flex-end;",
            },
        });
        ...
        ok.style.cssText =
            "padding:6px 14px;background:var(--interactive-accent);color:var(--text-on-accent);border:none;border-radius:5px;cursor:pointer;font-weight:700;";
        ok.addEventListener("click", () => {
            this.close();
            this.onInsert(this.opts);
        });
    }
    onClose() {
...
class ConfirmDeleteModal extends Modal {
    constructor(app, targetPage, onDelete) {
        super(app);
    ...
    onOpen() {
        ...
        const warn = el.createDiv({
            attr: {
                style: "background:#fee2e2;color:#991b1b;padding:12px;border-radius:6px;margin-bottom:16px;",
            },
        });
        ...
        warn.createSpan({
            text:
                ...
                this.targetPage +
                " and all annotations on it permanently.",
        });
        const row = el.createDiv({
            attr: {
        ...
        ok.style.cssText =
            "padding:6px 14px;background:#dc2626;color:#fff;border:none;border-radius:5px;cursor:pointer;font-weight:700;";
        ok.addEventListener("click", () => {
            this.close();
            this.onDelete(this.targetPage);
        });
    }
    onClose() {
        this.contentEl.empty();
...
class CreatePdfModal extends Modal {
    ...
    onOpen() {
        ...
        new Setting(el).setName("Filename").addText((t) =>
        ...
        new Setting(el).setName("Save this folder as default").addToggle((t) =>
            t.setValue(this.saveAsDefault).onChange((v) => {
                this.saveAsDefault = v;
            }),
        );
...
class DeletePageModal extends Modal {
    ...
    onOpen() {
        ...
        new Setting(el)
            .setName("Page to delete")
            .setDesc(`Total: ${this.total} pages`)
            .addText((t) => {
...
class PdfNotesView extends ItemView {
    constructor(leaf, plugin) {
        ...
        this.curPreview = null; // Shape state
        this.penId = null;
        this.lastMid = null;
    ...
    getDisplayText() {
        return this.pdfFile?.basename ?? "PDF.notes";
    }
    getIcon() {
        return "pencil";
    }
    ...
    getTotalScale() {
        return (this._renderScale || 1) * (this.scale || 1);
    }
    ...
    buildUI() {
        ...
        this.cameraEl = this.scrollEl.createDiv();
        this.cameraEl.addClass("pdf-notes-camera");
        this.cameraEl.style.cssText =
            "position:relative;display:flex;flex-direction:column;align-items:center;width:100%;transform-origin: 0 0;will-change: transform;";
    ...
    buildToolbar(tb) {
        ...
        const b = (lbl, title, idOrFn, fnOrXtra, maybeXtra = "") => {
            ...
            if (
                id &&
                !this.plugin.settings.enabledTools.includes(id) &&
                id !== "shapes"
            )
                return { style: {} };
        ...
        this.toggleMenu = (menu, anchor) => {
            ...
            if (menu.style.display !== "none" && menu.style.display !== "") {
                ...
                document
                    .querySelectorAll(".pdf-notes-menu")
                    .forEach((m) => (m.style.display = "none"));
                const rect = anchor.getBoundingClientRect();
                const isRight =
                    this.plugin.settings.toolbarPosition === "right";
                if (isRight) {
                    ...
                    menu.style.left =
                        Math.max(
                            10,
                            Math.min(rect.left, window.innerWidth - 180),
                        ) + "px";
        ...
        if (this.plugin.settings.enabledTools.includes("scroll"))
            this.btnHand = b("scroll", "Scroll / Hand Tool", "scroll", () =>
                this.setTool("scroll"),
            );
        if (this.plugin.settings.enabledTools.includes("select"))
            this.btnSelect = b("select", "Select / Move", "select", () =>
                this.setTool("select"),
            );
        if (this.plugin.settings.enabledTools.includes("lasso"))
            this.btnLasso = b("lasso", "Lasso Selection", "lasso", () =>
                this.setTool("lasso"),
            );
        ...
        if (this.plugin.settings.enabledTools.includes("pen")) {
            this.btnPen = b(
                "pen",
                "Pen – Click for color, width & opacity",
                "pen",
                () => {
                    this.setTool("pen");
            ...
            const createRow = (label) => {
                const r = this._penMenu.createDiv({
                ...
                l.createSpan({
                    text: label,
                    style: `font-size:10px;color:${T.mutedText};font-weight:bold;text-transform:uppercase;`,
                });
                const v = l.createSpan({
                    style: `font-size:10px;color:${T.valueText};`,
                });
                return { r, v };
            };

            const { r: cR } = createRow("Color");
            this.colorInput = cR.createEl("input", {
                type: "color",
                style: `width:100%;height:24px;padding:1px;background:${T.inputBg};border:1px solid ${T.btnBorder};border-radius:4px;cursor:pointer;`,
            });
            ...
            this.colorInput.oninput = (e) => {
                ...
                if (this.selectedElements.length > 0) {
                    this.selectedElements.forEach((sel) => {
                        if (sel.s.type !== "image") sel.s.color = this.color;
                    });
                    const pns = new Set(
                        this.selectedElements.map((sel) => sel.pn),
                    );
            ...
            const tS = tR.createEl("input", {
                type: "range",
                style: "width:100%;",
            });
            tS.min = "1";
            tS.max = "15";
            tS.value = String(this.lineWidth);
            tS.oninput = (e) => {
            ...
            const oS = oR.createEl("input", {
                type: "range",
                style: "width:100%;",
            });
            oS.min = "1";
            oS.max = "100";
            oS.value = String(Math.round(this.opacity * 100));
            oS.oninput = (e) => {
        ...
        if (this.plugin.settings.enabledTools.includes("eraser")) {
            ...
            this._eraseMenu = document.body.createDiv({
                cls: "pdf-notes-menu",
            });
            ...
            // Eraser Mode Toggle
            const modeRow = this._eraseMenu.createDiv({
                style: "display:flex;flex-direction:column;gap:4px;",
            });
            modeRow.createSpan({
                text: "Mode",
            ...
            const modeWrap = modeRow.createDiv({
            ...
            const updateModeBtns = () => {
                const isStroke =
                    (this.plugin.settings.eraserMode || "stroke") === "stroke";
            ...
            // Eraser Size
            const sizeRow = this._eraseMenu.createDiv({
                style: "display:flex;flex-direction:column;gap:4px;",
            });
            sizeRow.createSpan({
                text: "Size",
            ...
            const sIn = sizeRow.createEl("input", {
                type: "range",
                style: "width:100%;",
            });
            sIn.min = "5";
            ...
            sIn.oninput = (e) => {
                this.plugin.settings.eraserSize = parseInt(e.target.value);
                this.plugin.saveSettings();
            };
        }
        ...
        if (this.plugin.settings.enabledTools.includes("text")) {
            this.btnText = b(
                "text",
                "Text – Click for color, size & font",
                "text",
                () => {
                    this.setTool("text");
            ...
            const createRow = (label) => {
                const r = this._textMenu.createDiv({
                    style: "display:flex;flex-direction:column;gap:4px;",
                });
                const l = r.createDiv({
                    style: "display:flex;justify-content:space-between;align-items:center;",
            ...
            const textColorInput = cR.createEl("input", {
                type: "color",
                style: `width:100%;height:24px;padding:1px;background:${T.inputBg};border:1px solid ${T.btnBorder};border-radius:4px;cursor:pointer;`,
            });
            ...
            const sizeIn = sR.createEl("input", {
                type: "range",
                style: "width:100%;",
            });
            sizeIn.min = "8";
            sizeIn.max = "72";
            sizeIn.value = String(this.plugin.settings.textSize || 16);
            sizeIn.oninput = (e) => {
            ...
            const fontIn = fR.createEl("select", {
                style: `width:100%;background:${T.inputBg};color:${T.selectText};border:1px solid ${T.btnBorder};border-radius:4px;font-size:12px;`,
            });
            [
                "Inter",
                "Roboto",
            ...
            const otS = otR.createEl("input", {
                type: "range",
                style: "width:100%;",
            });
            otS.min = "1";
            otS.max = "100";
            otS.value = String(Math.round(this.opacity * 100));
            otS.oninput = (e) => {
                this.opacity = parseInt(e.target.value) / 100;
                otV.textContent = Math.round(this.opacity * 100) + "%";
                this.setTool("text");
        ...
        if (this.plugin.settings.enabledTools.includes("link")) {
            this.btnLink = b(
                "link",
                "Link Selection to Vault Page",
                "link",
                () => {
                    if (this.selectedElements.length === 0) {
                        new Notice(
                            "Please select elements first to link them.",
                        );
                        return;
                    }
        ...
        if (this.plugin.settings.enabledTools.includes("image"))
            this.btnImg = b("image", "Insert Image", "image", () =>
                this.setTool("image"),
            );
        if (this.plugin.settings.enabledTools.includes("snip"))
            this.btnSnip = b("snip", "Snip / Screenshot", "snip", () =>
                this.setTool("snip"),
            );
        ...
        // Group 4: Shapes
        const enabledShapes = ["rect", "circle", "line", "arrow"].filter((t) =>
            this.plugin.settings.enabledTools.includes(t),
        );
        if (enabledShapes.length > 0) {
            this.btnShapes = b(
                ...
                "shapes",
                () => this.toggleMenu(this._shapeMenu, this.btnShapes),
            );
            this.btnShapes.classList.add("has-dropdown");
            this._shapeMenu = document.body.createDiv({
                cls: "pdf-notes-menu",
            });
            ...
            enabledShapes.forEach((s) => {
                const sb = b(s, s, s, () => {
                    this.setTool(s);
                    this._shapeMenu.style.display = "none";
                });
        ...
        if (this.plugin.settings.enabledTools.includes("delete")) {
            this.btnDelete = b("delete", "Delete Selection", "delete", () =>
                this.deleteSelected(),
            );
        }
        ...
        this.btnFlatten = b("flatten", "Flatten PDF (Burn In)", "flatten", () =>
            this.flattenToPdf(),
        );
        this.btnFlatten.style.background = T.flattenBg;
        this.btnFlatten.style.color = T.flattenFg;

        this.btnExport = b("export", "Export PDF Copy", "export", () =>
            this.exportToPdf(),
        );
        if (this.btnExport && this.btnExport.style) {
            this.btnExport.style.background = T.saveBg;
            this.btnExport.style.color = T.saveFg;
        }
    }
    div(p) {
        const isRight = this.plugin.settings.toolbarPosition === "right";
        const T =
            this._theme ||
            getThemeColors(this.plugin.settings.themeMode || "dark");
        const d = p.createDiv();
        d.style.cssText = isRight
            ? `width:14px;height:1px;background:${T.border};margin:3px 0;opacity:0.5;`
            : `width:1px;height:14px;background:${T.border};margin:0 3px;opacity:0.5;`;
    }
    toggleZen() {
        const root =
            this.contentEl.closest(".pdf-notes-container") || this.contentEl;
        ...
        if (isZen) {
            this._oldS = {
                left: document.querySelector(".workspace-ribbon.side-dock-left")
                    ?.style.display,
                right: document.querySelector(
                    ".workspace-ribbon.side-dock-right",
                )?.style.display,
            };
            document
                .querySelectorAll(".workspace-ribbon")
                .forEach((r) => (r.style.display = "none"));
            document
                .querySelectorAll(".workspace-tab-header-container")
    ...
    updateToolbar() {
        ...
        if (this.btnShapes) {
            const isShape = ["rect", "circle", "line", "arrow"].includes(
                this.tool,
            );
            ...
            if (isShape) {
                if (this.plugin.settings.iconType === "lucide") {
                    ...
                    this.btnShapes.createSpan({
                        text: " ▾",
                        attr: {
                            style: "font-size:8px;margin-left:3px;opacity:0.7;",
                        },
                    });
                } else {
                    this.btnShapes.innerHTML =
                        icons[this.tool] +
                        ' <span style="font-size:8px;margin-left:3px;opacity:0.7;">▾</span>';
                }
            }
        }
        if (this.btnDelete) {
            const hasSelection = this.selectedElements.length > 0;
            this.btnDelete.style.background = hasSelection
                ? T.deleteBg + "33"
                : "transparent";
        ...
        if (this.zoomInfo)
            this.zoomInfo.textContent =
                Math.round(this.getTotalScale() * 100) + "%";
        Object.values(this.pageCanvases).forEach(({ inkCanvas }) =>
            this.setCursor(inkCanvas),
        );
    }
    setCursor(c) {
        if (!c) return;
    ...
    _getZoomAnchor(anchorEvent = null) {
        if (!this.scrollEl || !this.cameraEl) return null;
        ...
        const fallback =
            anchorEvent && typeof anchorEvent.clientX === "number"
                ...
                : this._lastCursorPos || {
                      clientX:
                          this.scrollEl.getBoundingClientRect().left +
        ...
        const target = document
            .elementFromPoint(fallback.clientX, fallback.clientY)
            ?.closest("[data-page]");
        const pageEl =
            target ||
            this.cameraEl.querySelector(`[data-page="${this.visiblePage}"]`);
        if (!pageEl) return null;

        const rect = pageEl.getBoundingClientRect();
        return {
            page: parseInt(pageEl.dataset.page),
            clientX: fallback.clientX,
            clientY: fallback.clientY,
            relX: fallback.clientX - rect.left,
            relY: fallback.clientY - rect.top,
        };
    }

    zoom(d, anchorEvent = null) {
        const prevScale = this.scale;
        const next =
            Math.round(Math.max(0.2, Math.min(8.0, prevScale + d)) * 10) / 10;
        if (next === prevScale) return;

        const anchor = this._getZoomAnchor(anchorEvent);
        const cameraRect = this.cameraEl?.getBoundingClientRect();

        // anchor in *local* (unscaled) content coords, using the OLD scale/translate
        let ax = cameraRect ? cameraRect.width / 2 : 0;
        let ay = cameraRect ? cameraRect.height / 2 : 0;
        if (anchor && cameraRect) {
            ax = (anchor.clientX - cameraRect.left) / prevScale;
            ay = (anchor.clientY - cameraRect.top) / prevScale;
        }

        // keep the local point (ax, ay) fixed under the cursor at the new scale
        this.tx -= ax * (next - prevScale);
        this.ty -= ay * (next - prevScale);
        this.scale = next;
        this.updateToolbar();

        if (this.cameraEl) {
            this.cameraEl.style.transformOrigin = "0 0";
            this.cameraEl.style.transform = `translate(${this.tx}px, ${this.ty}px) scale(${this.scale.toFixed(4)})`;
        }
    }
    ...
    async rebuildAll() {
        ...
        try {
            ...
            const renderScale = this._renderScale || 1.5;
            let estW = 600 * renderScale,
                estH = 800 * renderScale;
            if (!Object.keys(this.pageSizes).length) {
                try {
                    const p1 = await this.pdfDoc.getPage(1);
                    const vp = p1.getViewport({ scale: renderScale });
                    estW = vp.width;
                    estH = vp.height;
                    p1.cleanup();
                } catch (e) {}
            ...
            for (let i = 1; i <= this.pageCount; i++) {
                let wrap = this.cameraEl.querySelector(`[data-page="${i}"]`);
                const w = this.pageSizes[i]?.width ?? estW;
                const h = this.pageSizes[i]?.height ?? estH;

                if (!wrap) {
                    wrap = this.cameraEl.createDiv();
            ...
            if (this.statusEl)
                this.statusEl.setText(
                    `${this.pageCount} Pages | Zoom: ${Math.round(this.getTotalScale() * 100)}%`,
                );
            this.updateToolbar();
        } finally {
    ...
    _setupVisibilityObserver() {
        ...
        this.scrollEl
            ...
            .forEach((el) => this._visObs.observe(el));
    }
    ...
    _unloadPage(num) {
        ...
        if (d.cacheCanvas) {
            d.cacheCanvas.width = 0;
            d.cacheCanvas.height = 0;
        }
        delete this.pageCanvases[num];
        this._renderedPages?.delete(num);
        const wrap =
            d.wrap || this.scrollEl.querySelector(`[data-page="${num}"]`);
        if (wrap) {
    ...
    async renderPageInWrap(num, wrap) {
        ...
        try {
            ...
            const vp = page.getViewport({ scale: this._renderScale || 1.5 });
            ...
            lc.style.inset = "0";
            lc.style.zIndex = "6"; // On top of inkCanvas
    ...
    updateCache(pn) {
        ...
        if (!d || !d.cacheCanvas) return;
        const ctx = d.cacheCanvas.getContext("2d");
        ctx.clearRect(0, 0, d.vp.width, d.vp.height);
        ctx.save();
        ctx.scale(this._renderScale || 1.5, this._renderScale || 1.5);
    ...
    setupGlobalEvents() {
        ...
        this._onGlobalClick = (e) => {
            if (
                !e.target.closest(".pdf-notes-menu") &&
        ...
        this._onKeyDown = (ev) => {
            ...
            if (!ev.ctrlKey && !ev.metaKey && !ev.altKey) {
                ...
                const tool = hotkeys[ev.key.toLowerCase()];
                if (tool) {
                    ev.preventDefault();
                    if (tool === "delete") this.deleteSelected();
                    else if (tool === "undo") this.undo();
                    else if (tool === "redo") this.redo();
                    else if (tool === "flatten") this.flattenToPdf();
                    else if (tool === "export") this.exportToPdf();
                    else if (tool === "zen") this.toggleZen();
            ...
            if (ev.ctrlKey && ev.key === "z" && !ev.shiftKey) {
                ...
                (ev.ctrlKey && ev.key === "y") ||
                (ev.ctrlKey && ev.shiftKey && ev.key === "Z")
            ) {
                ev.preventDefault();
                this.redo();
        ...
        this._onZoomPointerMove = (e) => {
            if (e.pointerType === "touch") return;
            this._lastCursorPos = { clientX: e.clientX, clientY: e.clientY };
        };
        el.addEventListener("pointermove", this._onZoomPointerMove);
        el.addEventListener("touchmove", this._touchMoveGuard, {
            passive: false,
        });
        const getPageAt = (e) => {
        ...
        const onS = async (e) => {
            ...
            const viewScale = this.getTotalScale();
            const p = {
            ...
            if (useTool === "text") {
                ...
                const fontSize =
                    (this.plugin.settings.textSize || 16) * viewScale;
                const sy = e.clientY - fontSize / 4;
                p.y = (sy - r.top) / viewScale; // Align physical storage to visual offset
            ...
            if (useTool === "image") {
                const insertImage = (dataUrl) => {
                    ...
                    img.onload = () => {
                        ...
                        const imgScale = this.getTotalScale();
                        const w = Math.min(200, img.width / imgScale);
                        const s = {
                            type: "image",
                            data: dataUrl,
                            x: p.x,
                        ...
                        (this.strokes[page.pn] =
                            this.strokes[page.pn] || []).push(s);
                ...
                if (src === "vault") {
                    new VaultImagePicker(
                        this.app,
                        this.plugin.settings.imageVaultFolder || "",
                        async (file) => {
            ...
            if (useTool === "select") {
                ...
                const now = Date.now();
                const isDblClick =
                    this._lastSelectClick &&
                    now - this._lastSelectClick.time < 400 &&
                    Math.hypot(
                        p.x - this._lastSelectClick.x,
                        p.y - this._lastSelectClick.y,
                    ) < 10;
                this._lastSelectClick = {
                    time: now,
                    x: p.x,
                ...
                if (hit) {
                    ...
                    const inSelected = this.selectedElements.some((sel) => {
                        ...
                        if (s.type === "circle")
                            return (
                                Math.hypot(p.x - s.cx, p.y - s.cy) <
                                Math.max(s.rx, s.ry)
                            );
                        if (s.type === "line" || s.type === "arrow")
                            return this.shapeHitTest(s, p, 15);
                        if (s.points) {
        ...
        const onM = (e) => {
            if (e.pointerType === "touch") {
                ...
                if (this.isPinching && this.activeTouches.size === 2) {
                    ...
                    this.scale = Math.max(
                        0.1,
                        Math.min(8.0, this.pinchScale * delta),
                    );
                    if (this.cameraEl) {
                        const midX = (t[0].clientX + t[1].clientX) / 2;
                        const midY = (t[0].clientY + t[1].clientY) / 2;
                        const cameraRect =
                            this.cameraEl.getBoundingClientRect();
                        const ax = (midX - cameraRect.left) / prevScale;
                        const ay = (midY - cameraRect.top) / prevScale;
                        this.tx -= ax * (this.scale - prevScale);
                        this.ty -= ay * (this.scale - prevScale);
                        this.cameraEl.style.transformOrigin = "0 0";
                        this.cameraEl.style.transform = `translate(${this.tx}px, ${this.ty}px) scale(${this.scale.toFixed(4)})`;
                        this.cameraEl.style.transition = "";
                    }
                    this.updateToolbar();
                    return;
                }
                if (this.activeTouches.size === 1 && this.touchScrollStart) {
                    el.scrollTop =
                        this.touchScrollStart.top -
                        (e.clientY - this.touchScrollStart.y);
                    el.scrollLeft =
                        this.touchScrollStart.left -
                        (e.clientX - this.touchScrollStart.x);
                    return;
                }
            }
            ...
            if (!this.drawing || e.pointerId !== this.penId) {
                if (this.isHandScrolling && this.handScrollStart) {
                    el.scrollTop =
                        this.handScrollStart.top -
                        (e.clientY - this.handScrollStart.y);
                    el.scrollLeft =
                        this.handScrollStart.left -
                        (e.clientX - this.handScrollStart.x);
                }
                return;
            }
            const canvas = document.querySelector(
                `[data-page="${this.activePn}"] canvas:last-child`,
            );
            if (!canvas) return;
            const r = canvas.getBoundingClientRect();
            const viewScale = this.getTotalScale();
            ...
            if (useTool === "select") {
                if (this.isResizing && this.activeHandle) {
                    ...
                    if (this.activeHandle.type === "image") {
                        const aspect =
                            this.activeHandle.w / this.activeHandle.h;
                        this.activeHandle.w = Math.max(
                            10,
                            this.activeHandle.w + dx,
                        );
                        this.activeHandle.h = this.activeHandle.w / aspect;
                    } else if (this.activeHandle.type === "text") {
                        const oldSize = this.activeHandle.size || 16;
                        this.activeHandle.size = Math.max(
                            8,
                            Math.min(150, oldSize + (dx + dy) / 2),
                        );
                    } else {
                        this.activeHandle.w = Math.max(
                            10,
                        ...
                        this.activeHandle.h = Math.max(
                            10,
                            this.activeHandle.h + dy,
                        );
                    }
                    this.moveStart = p;
                ...
                this.snipRect = {
                    x: Math.min(this.snipStart.x, p.x),
                    y: Math.min(this.snipStart.y, p.y),
                    w: Math.abs(p.x - this.snipStart.x),
                    h: Math.abs(p.y - this.snipStart.y),
                };
                this.requestRedraw(this.activePn);
                ...
                const lctx = d?.liveCanvas
                    ? d.liveCanvas.getContext("2d")
                    : null;
                for (const ce of coalesced) {
                    const cp = {
                        x: (ce.clientX - r.left) / viewScale,
                        y: (ce.clientY - r.top) / viewScale,
                    };
                    const pts = this.curStroke.points;
                    const last = pts[pts.length - 1];
                    if (
                        Math.hypot(cp.x - last.x, cp.y - last.y) >
                        0.1 / viewScale
                    ) {
                        if (lctx) {
                            lctx.save();
                ...
                if (!this._eraseRaf) {
                    this._eraseRaf = requestAnimationFrame(() => {
                        ...
                        if (this._erasePending) {
                            this.erase(
                                this._erasePending.pn,
                                this._erasePending.pos,
                            );
                ...
                ["rect", "circle", "line", "arrow"].includes(useTool) &&
                this.shapeStart
            ) {
        ...
        const onE = (e) => {
            if (e.pointerType === "touch") {
                ...
                if (this.isPinching && this.activeTouches.size < 2) {
                    this.isPinching = false;
                    if (this.cameraEl) this.cameraEl.style.transition = "";
                }
                this.touchScrollStart = null;
                return;
            }
            if (this.drawing && e.pointerId === this.penId) {
                ...
                if (useTool === "select") {
                    ...
                    this.activeHandle = null;
                    this.moveStart = null;
                } else if (useTool === "lasso" && this.lassoRect) {
                    this.completeLasso();
                    this.lassoRect = null;
                    ...
                    (this.strokes[this.activePn] =
                        this.strokes[this.activePn] || []).push(this.curStroke);
                    this.pushUndo(this.activePn, this.curStroke);
                } else if (this.curPreview) {
                    (this.strokes[this.activePn] =
                        this.strokes[this.activePn] || []).push(
                        this.curPreview,
                    );
        ...
        el.addEventListener("pointerup", onE);
        el.addEventListener("pointerleave", onE);
        el.addEventListener("pointercancel", onE);
        ...
        el.addEventListener(
            ...
            (e) => {
                if (e.ctrlKey) {
                    e.preventDefault();
                    this.zoom(e.deltaY > 0 ? -0.1 : 0.1, e);
                }
            },
            { passive: false },
        );
    ...
    drawShape(ctx, s) {
        ...
        ctx.globalAlpha = s.opacity ?? 1.0;
        ctx.strokeStyle = s.color;
        ctx.lineWidth = s.lineWidth || s.width || 2;
        ctx.lineCap = "round";
        ctx.lineJoin = "round";
        ...
        if (s.type === "text") {
            ...
            const a = Math.atan2(s.y2 - s.y1, s.x2 - s.x1),
                sz = Math.max(8, (s.lineWidth || 2) * 5);
            ctx.beginPath();
            ctx.moveTo(s.x2, s.y2);
            ctx.lineTo(
                s.x2 - sz * Math.cos(a - Math.PI / 6),
                s.y2 - sz * Math.sin(a - Math.PI / 6),
            );
            ctx.lineTo(
                s.x2 - sz * Math.cos(a + Math.PI / 6),
                s.y2 - sz * Math.sin(a + Math.PI / 6),
    ...
    redrawWithPreview(pn) {
        ...
        if (this.curStroke && this.curStroke.pn === pn)
            this.drawShape(ctx, this.curStroke);
        const useTool = this.activeStrokeTool || this.tool;
    ...
    hitTest(pn, p) {
        ...
        for (let i = list.length - 1; i >= 0; i--) {
            ...
            if (s.type === "text") {
                ...
                if (Math.hypot(p.x - s.cx, p.y - s.cy) < Math.max(s.rx, s.ry))
                    return s;
            } else if (s.type === "line" || s.type === "arrow") {
                if (this.shapeHitTest(s, p, 10)) return s;
                ...
                if (
                    s.points.some(
                        (pt) => Math.hypot(pt.x - p.x, pt.y - p.y) < threshold,
                    )
                )
                    return s;
            }
    ...
    startEditingText(s, pn, e) {
        ...
        const canv = document.querySelector(
            `[data-page="${pn}"] canvas:last-child`,
        );
        const r = canv.getBoundingClientRect();
        const viewScale = this.getTotalScale();

        const fontSize = (s.size || 16) * viewScale;
        const sx = s.x * viewScale + r.left;
        const sy = s.y * viewScale + r.top;
    ...
    finishFloatingInput() {
        ...
        if (mode === "new") {
            ...
            if (txt.trim() === "") {
                this.strokes[pn] = this.strokes[pn].filter(
                    (item) => item !== s,
                );
            } else {
                s.text = txt;
        ...
        try {
            tmp.toBlob(async (blob) => {
                ...
                try {
                    ...
                    new Notice(
                        "Screenshot copied to clipboard & added to page!",
                    );
                } catch (err) {
    ...
    async pasteFromClipboard() {
        try {
            ...
            new Notice(
                "Paste failed. Obsidian may need clipboard permissions.",
            );
        }
    }
        ...
        rd.onload = () => {
            ...
            img.onload = () => {
                ...
                if (wrap) {
                    ...
                    const relativeY = (viewCenter - wrapRect.top) / viewScale;

                    x =
                        (scrollRect.width / 2 -
                            (wrapRect.left - scrollRect.left)) /
                        viewScale;
                    y = Math.max(
                        50,
                        Math.min(relativeY, wrapRect.height / viewScale - 50),
                    );
                }

                const viewScale = this.getTotalScale();
                const w = Math.min(250, img.width / viewScale);
                const s = {
                    type: "image",
                    data: rd.result,
    ...
    cropSelected() {
        ...
        if (sel.s.type !== "image") return;
        const s = sel.s;
        const msg =
            "Enter crop margins (top left bottom right) in pixels or click cancel";
    ...
    shapeHitTest(s, pos, r) {
        ...
        if (s.type === "rect")
            return (
                ...
                pos.y >= s.y - rt &&
                pos.y <= s.y + s.h + rt &&
                (Math.abs(pos.x - s.x) < rt ||
                    Math.abs(pos.x - s.x - s.w) < rt ||
                    Math.abs(pos.y - s.y) < rt ||
                    Math.abs(pos.y - s.y - s.h) < rt)
            );
        if (s.type === "circle") {
            const d = Math.hypot(
                (pos.x - s.cx) / (s.rx || 1),
            ...
            return Math.abs(d - 1) < rt / Math.max(s.rx || 1, s.ry || 1);
        }
        if (s.type === "line" || s.type === "arrow") {
            const dx = s.x2 - s.x1,
                dy = s.y2 - s.y1,
                l2 = dx * dx + dy * dy;
            if (!l2) return Math.hypot(pos.x - s.x1, pos.y - s.y1) < rt;
            const t = Math.max(
                0,
                Math.min(1, ((pos.x - s.x1) * dx + (pos.y - s.y1) * dy) / l2),
            );
            return (
                Math.hypot(pos.x - s.x1 - t * dx, pos.y - s.y1 - t * dy) < rt
            );
        }
        return false;
    }
        ...
        this.strokes[pn] = this.strokes[pn].filter((s) => {
            if (!s.type || s.type === "stroke")
                return !s.points.some(
                    (p) => Math.hypot(p.x - pos.x, p.y - pos.y) < r,
                );
    ...
    erase(pn, pos) {
        if ((this.plugin.settings?.eraserMode ?? "stroke") === "pixel")
            this.erasePixel(pn, pos);
        else this.eraseStroke(pn, pos);
    }
    pushUndo(pn, item) {
    ...
    scheduleSave() {
        ...
        this.saveTimer = setTimeout(() => this.saveAsAnnotations(), 5000);
    }

    async buildFlattenedPdfBytes(sourceBytes) {
        const pdfLib = PDFLib;
        const {
            PDFDocument,
            rgb,
                ...
                if (annotArr) {
                    ...
                    for (let j = 0; j < annotArr.size(); j++) {
                        try {
                            ...
                            let nmObj = ann?.lookupMaybe(
                                PDFName.of("NM"),
                                PDFString,
                            );
                            if (!nmObj) {
                                nmObj = ann?.lookupMaybe(
                                    PDFName.of("NM"),
                                    PDFHexString,
                                );
                            }
                            let nmStr = nmObj
                                ...
                                : null;
                            if (!nmStr) {
                                const n2 = ann?.lookupMaybe(
                                    PDFName.of("NM"),
                                    PDFName,
                                );
                                if (n2) nmStr = n2.asString();
                            }
                            if (
                                !nmStr ||
                                (!nmStr.startsWith("pdfnotes-") &&
                                    !nmStr.startsWith("pdfink-") &&
                                    !nmStr.startsWith("pdfstift-"))
                            ) {
                                keep.push(ref);
                            }
                        } catch (_) {}
            ...
            for (const s of strokes) {
                ...
                if (s.type === "text") {
                    ...
                    try {
                        ...
                        const pdfImg =
                            s.data.includes("jpeg") || s.data.includes("jpg")
                        ...
                        console.error("[PDF.notes] Image export error", e);
                    }
                } else if (!s.type || s.type === "stroke") {
                    ...
                    if (s.type === "line") {
                        pg.drawLine({
                            ...
                            color: co,
                            opacity: op,
                            lineCap: LineCapStyle ? LineCapStyle.Round : 1,
                        });
                    } else {
                        const ang = Math.atan2(-(s.y2 - s.y1), s.x2 - s.x1),
                            sz = Math.max(6, Math.max(0.5, lw) * 4);
                        const p1x = s.x2 - sz * Math.cos(ang - Math.PI / 6),
                            p1y = y2 - sz * Math.sin(ang - Math.PI / 6);
                        const p2x = s.x2 - sz * Math.cos(ang + Math.PI / 6),
                            p2y = y2 - sz * Math.sin(ang + Math.PI / 6);
    ...
    async saveAsAnnotations() {
        ...
        try {
            const pdfLib = PDFLib;
            const { PDFDocument, PDFName, PDFArray, PDFDict, PDFString } =
                pdfLib;
            // Reuse cached clean PDF buffer if available, avoids re-reading 40MB+ from disk
            const buf =
                this._cleanPdfBuf ||
                (await this.app.vault.adapter.readBinary(this.pdfFile.path));
            ...
            for (const [pgStr, strokes] of Object.entries(this.strokes)) {
                ...
                let annotArr;
                try {
                    annotArr = page.node.lookup(PDFName.of("Annots"), PDFArray);
                } catch (_) {}
                if (annotArr) {
                    const keep = [];
                    for (let j = 0; j < annotArr.size(); j++) {
                ...
                for (let si = 0; si < strokes.length; si++) {
                    ...
                    annotObj.Contents = PDFString.of(
                        this.shapeToContents(s, cvW, cvH),
                    ); // All metadata (color, opacity, shape) always in Contents
                    annotArr.push(
                        doc.context.register(doc.context.obj(annotObj)),
                    );
                }
            }
    ...
    async flattenToPdf() {
        if (!Object.values(this.strokes).some((a) => a?.length > 0)) {
            this.statusEl.setText("No drawings.");
            return;
        }
        if (
            !confirm(
                "🔥 Flatten drawings PERMANENTLY into the PDF?\n(Cannot be erased afterwards, even after restart)",
            )
        )
            return;
        this.statusEl.setText("🔥 Flattening...");
        if (this.btnFlatten) this.btnFlatten.disabled = true;
        try {
            const lib = PDFLib;
            const {
                PDFDocument,
            ...
            const finalBuffer = docBytes.buffer.slice(
                docBytes.byteOffset,
                docBytes.byteOffset + docBytes.byteLength,
            );
            await this.app.vault.adapter.writeBinary(
            ...
            new Notice("PDF.notes: Flattened 🔥");
            ...
            if (this.btnFlatten) this.btnFlatten.disabled = false;
        }
    }
    ...
    async exportToPdf() {
        ...
        try {
            ...
            const hasDrawings = Object.values(this.strokes).some(
                (items) => items?.length > 0,
            );
            const finalBuffer = hasDrawings
                ? await this.buildFlattenedPdfBytes(sourceBytes)
                : sourceBytes;
            const folder = nodePath.dirname(this.pdfFile.path);
            const base = this.pdfFile.basename || "exported-pdf";
            let exportPath = nodePath.join(folder, `${base} (export).pdf`);
            let idx = 1;
            while (this.app.vault.getAbstractFileByPath(exportPath)) {
                exportPath = nodePath.join(
                    folder,
                    `${base} (export ${idx++}).pdf`,
                );
            }
            await this.app.vault.adapter.writeBinary(exportPath, finalBuffer);
            this.statusEl.setText("✅ Exported!");
            new Notice(`📤 Exported PDF: ${exportPath}`);
        } catch (err) {
            this.statusEl.setText("❌ " + err.message);
            new Notice("Export error: " + err.message, 6000);
            console.error("[PDF.notes] Export error:", err);
        } finally {
    ...
    async loadPdfAndAnnotations() {
        try {
            ...
            try {
                    ...
                    for (let j = 0; j < annotArr.size(); j++) {
                        try {
                            ...
                            let r = 0,
                                ...
                                b = 0;
                            try {
                                const cArr = ann.lookup(
                                    PDFName.of("C"),
                                    PDFArray,
                                );
                                if (cArr?.size() >= 3) {
                                    r = cArr.get(0).asNumber();
                                    g2 = cArr.get(1).asNumber();
                                    b = cArr.get(2).asNumber();
                                }
                            } catch (_) {}
                            const toHex = (v) =>
                                Math.round(v * 255)
                            ...
                            try {
                                ...
                                if (bs) {
                                    const w = bs.lookupMaybe(PDFName.of("W"));
                                    if (w) lw = w.asNumber();
                                }
                            } catch (_) {}
                            let isShape = false;
                            let opacity = 1.0;
                            try {
                                let contentsObj = ann.lookupMaybe(
                                    PDFName.of("Contents"),
                                    PDFString,
                                );
                                if (!contentsObj)
                                    contentsObj = ann.lookupMaybe(
                                        PDFName.of("Contents"),
                                        pdfLib.PDFHexString,
                                    );
                                if (contentsObj) {
                            ...
                            const inkList = ann.lookup(
                                PDFName.of("InkList"),
                                PDFArray,
                            );
                            if (!inkList) continue;
                            for (let k = 0; k < inkList.size(); k++) {
                                const flatPts = inkList.lookup(k, PDFArray);
                                if (!flatPts) continue;
                                const points = [];
                                for (
                                    let p = 0;
                                    ...
                                    try {
                                        points.push({
                                            x: flatPts.get(p).asNumber(),
                                            y:
                                                pdfH -
                                                flatPts.get(p + 1).asNumber(),
                                        });
                                    } catch (_) {}
                                }
                                if (points.length >= 2) {
                ...
                console.log(
                    "[PDF.notes] Annotations loaded:",
                    Object.values(this.strokes).flat().length,
                    "strokes",
                );
            } catch (innerErr) {
                console.warn(
                    "[PDF.notes] Error preprocessing annotations:",
                    innerErr.message,
                );
            ...
            this._cleanPdfBuf = u8ForPdfJs.buffer.slice(
                u8ForPdfJs.byteOffset,
                u8ForPdfJs.byteOffset + u8ForPdfJs.byteLength,
            );
            ...
            console.error(
                "[PDF.notes] loadPdfAndAnnotations fatal failure:",
                e,
            );
    ...
    async insertPage({ position, pageType, targetPage }) {
        ...
        try {
            ...
            const origBytes = await this.app.vault.adapter.readBinary(
                this.pdfFile.path,
            );
            ...
            if (ri >= 0) {
                const sz = srcDoc.getPage(ri).getSize();
                rW = sz.width;
                rH = sz.height;
            }
            ...
            for (let i = idx; i < srcDoc.getPageCount(); i++)
                newDoc.addPage(allCopied[i]);
            ...
            new Notice(
                `📄 Page inserted (${PAGE_TYPES.find((p) => p.id === pageType)?.label || pageType}) ✅`,
            );
            ...
            // Scroll to new page
            const scrollTo =
                position === "before" ? targetPage : targetPage + 1;
            setTimeout(() => {
                this.scrollEl
                    .querySelector(`[data-page="${scrollTo}"]`)
                    ?.scrollIntoView({ behavior: "smooth", block: "start" });
            }, 300);
        } catch (err) {
            this.statusEl.setText("❌ " + err.message);
    ...
    openDeleteModal() {
        if (this.pageCount <= 1) {
            new Notice("A PDF must have at least one page.");
            return;
        }
        new DeletePageModal(this.app, this.visiblePage, this.pageCount, (pg) =>
            this.deletePage(pg),
        ).open();
    }
    ...
    async deletePage(pageNum) {
        if (pageNum < 1 || pageNum > this.pageCount) {
            new Notice("Invalid page number.");
            return;
        }
        if (this.pageCount <= 1) {
            new Notice("Cannot delete the last page.");
            return;
        }
        ...
        try {
            ...
            setTimeout(() => {
                this.scrollEl
                    .querySelector(`[data-page="${scrollTarget}"]`)
                    ?.scrollIntoView({ behavior: "smooth", block: "start" });
            }, 300);
        } catch (err) {
            this.statusEl.setText("❌ " + err.message);
    ...
    async getPdfJs() {
        if (window.pdfjsLib) {
            if (!window.pdfjsLib.GlobalWorkerOptions.workerSrc) {
                ...
                try {
                    const r =
                        window.require && window.require("@electron/remote");
                    if (r) {
                        const ap = r.app.getAppPath();
                        const wPath = nodePath.join(
                            ap,
        ...
        try {
            ...
            if (r) {
                const ap = r.app.getAppPath();
                for (const c of [
                    nodePath.join(ap, "node_modules/pdfjs-dist/build/pdf.js"),
                    nodePath.join(
                        ap,
                        "../app.asar.unpacked/node_modules/pdfjs-dist/build/pdf.js",
                    ),
                ]) {
                    if (fs.existsSync(c)) return window.require(c);
                }
            }
        } catch (_) {}
        try {
            return window.require("pdfjs-dist");
        } catch (_) {}
        return null;
    }
    ...
    async onClose() {
        ...
        if (this._onKeyDown)
            window.removeEventListener("keydown", this._onKeyDown);
        if (this._onPaste) window.removeEventListener("paste", this._onPaste);
        if (this._onZoomPointerMove && this.scrollEl)
            this.scrollEl.removeEventListener(
                "pointermove",
                this._onZoomPointerMove,
            );
        if (this.scrollEl && this._touchMoveGuard)
            this.scrollEl.removeEventListener(
                "touchmove",
                this._touchMoveGuard,
            );
        if (this._scrollRaf) {
...
class PdfNotesSettingTab extends PluginSettingTab {
    ...
    display() {
        ...
        banner.style.cssText =
            "padding:15px; background: rgba(255, 100, 0, 0.1); border: 1px solid #ff6400; border-radius: 8px; margin-bottom: 20px;";
        banner.createEl("h3", {
            text: "⚠️ Early Version",
            attr: { style: "margin: 0 0 10px 0; color: #ff6400;" },
        });
        banner.createEl("p", {
        ...
        new Setting(el)
            ...
            .setDesc(
                "Folder where new PDFs will be stored by default (empty = Vault root).",
            )
            .addText((t) => {
                t.setValue(this.plugin.settings.newPdfFolder).onChange(
                    async (v) => {
        ...
        new Setting(el)
            ...
            .addButton((b) =>
                b
                    .setButtonText("➕ New PDF")
                    .setCta()
                    .onClick(() => this.plugin.createNewPdf()),
            );

        el.createEl("h3", { text: "💾 Backup on page insertion" });
        el.createEl("p", {
            text: "A backup copy of the PDF is created before every insertion or deletion operation.",
            attr: {
                style: "color:var(--text-muted);font-size:13px;margin-bottom:10px;",
            },
        });
            ...
            .addDropdown((dd) => {
                dd.addOption("beside", "📂 Next to the PDF file")
                    ...
                    .onChange(async (v) => {
                        ...
                        await this.plugin.saveSettings();
                        if (folderRow)
                            folderRow.settingEl.style.display =
                                v === "folder" ? "" : "none";
                    });
            });
        ...
        folderRow = new Setting(el)
            ...
            .setDesc(
                "Absolute path to the destination folder (will be created automatically if needed).",
            )
            .addText((t) => {
        ...
        folderRow.settingEl.style.display =
            this.plugin.settings.backupMode === "folder" ? "" : "none";

        const info = el.createDiv();
        info.style.cssText =
            "margin-top:10px;padding:10px 14px;background:var(--background-secondary);border-radius:6px;font-size:12px;color:var(--text-muted);line-height:1.6;";
        info.createEl("b", { text: "Filename: " });
        info.createEl("code", { text: "filename.pdf.bak" });
        info.createEl("br");
        info.createSpan({
            text: "Overwritten with each insert/delete action (always state before the change).",
        });
        ...
        new Setting(el)
            .setName("Eraser size")
            .setDesc(
                "Radius of the eraser in pixels (10 = small · 20 = medium · 40 = large). Eraser mode (Strich/Punkt) is accessible via double-click on the eraser tool.",
            )
            .addSlider((sl) => {
                sl.setLimits(5, 80, 5)
        ...
        new Setting(el)
            ...
            .setDesc(
                "When disabled, only a pen or mouse can draw. Fingers will only be used for scrolling. (Recommended for laptop/tablet users with pen).",
            )
            .addToggle((tg) =>
                tg
        ...
        new Setting(el)
            ...
            .setDesc(
                "Which tool should be used while holding the pen side button? (Requires Pen support).",
            )
            .addDropdown((dd) => {
                dd.addOption("eraser", "🧽 Eraser")
                    ...
                    .setValue(
                        this.plugin.settings.penSideButtonTool || "eraser",
                    )
                    .onChange(async (v) => {
        ...
        new Setting(el)
            ...
            .setDesc(
                "Where should images be loaded from when using the image tool?",
            )
            .addDropdown((dd) => {
                dd.addOption("filesystem", "📁 File system (file picker)")
                    ...
                    .onChange(async (v) => {
                        ...
                        await this.plugin.saveSettings();
                        if (vaultFolderRow)
                            vaultFolderRow.settingEl.style.display =
                                v === "vault" ? "" : "none";
                    });
            });
        ...
        vaultFolderRow = new Setting(el)
            ...
            .setDesc(
                "Only search for images in this folder (empty = entire Vault).",
            )
            .addText((t) => {
        ...
        vaultFolderRow.settingEl.style.display =
            (this.plugin.settings.imageSource || "filesystem") === "vault"
                ? ""
                : "none";
        ...
        const toolMap = {
            ...
            insert: "📄 Insert Page",
            remove: "🗑 Delete Page",
            export: "📤 Export PDF Copy",
            flatten: "🔥 Flatten PDF",
        };

        Object.entries(toolMap).forEach(([id, name]) => {
            new Setting(el).setName(name).addToggle((tg) =>
                tg
                    .setValue(this.plugin.settings.enabledTools.includes(id))
                    .onChange(async (v) => {
                        if (v) {
                            if (!this.plugin.settings.enabledTools.includes(id))
                                this.plugin.settings.enabledTools.push(id);
                        } else {
                            this.plugin.settings.enabledTools =
                                this.plugin.settings.enabledTools.filter(
                                    (t) => t !== id,
                                );
                        }
                        await this.plugin.saveSettings();
        ...
        new Setting(el)
            ...
            .setDesc(
                "Choose whether the PDF editor uses a light or dark background.",
            )
            .addDropdown((dd) => {
        ...
        emojiSection.style.display =
            (this.plugin.settings.iconType || "lucide") === "emoji"
                ? "block"
                : "none";
        ...
        new Setting(el)
            .setName("Icon style")
            .setDesc(
                "Lucide uses clean vector icons. Emoji lets you assign custom symbols to each tool.",
            )
            .addDropdown((dd) => {
        ...
        emojiSection.createEl("p", {
            text: "Choose which symbol/emoji is displayed for each tool in the toolbar. Select a preset or enter your own custom symbol.",
        ...
        if (!this.plugin.settings.toolIcons)
            this.plugin.settings.toolIcons = { ...DEFAULT_SETTINGS.toolIcons };

        Object.entries(toolMap).forEach(([id, name]) => {
            const currentIcon =
                this.plugin.settings.toolIcons[id] ||
                DEFAULT_SETTINGS.toolIcons[id] ||
                "?";
            ...
            row.addDropdown((dd) => {
                ...
                dd.onChange(async (v) => {
                    if (v === "__custom__") {
                        ...
                        const customInput = row.settingEl.querySelector(
                            ".pdf-icon-custom-input",
                        );
                        if (customInput) {
        ...
        asInfo.style.cssText =
            "padding:10px 14px;background:var(--background-secondary);border-radius:6px;font-size:12px;color:var(--text-muted);line-height:1.6;";
        asInfo.createSpan({ text: "✅ " });
        asInfo.createEl("b", { text: "Auto-Save is active. " });
        asInfo.createSpan({
            text: "Drawings are automatically written into the PDF ",
        });
        asInfo.createEl("b", { text: "5 seconds " });
        asInfo.createSpan({ text: "after the last stroke." });
        asInfo.createEl("br");
        asInfo.createSpan({
            text: "The 💾 button saves manually immediately.",
        });
        ...
        credits.createEl("br");
        credits.createSpan({
            text: "This is the first version of PDF.notes. ",
        });
        credits.createEl("b", {
            text: "Feedback and criticism are highly welcome",
        });
        ...
        social.style.cssText =
            "display:flex;flex-direction:column;gap:8px;padding:10px 14px;background:var(--background-secondary);border-radius:6px;font-size:13px;";
        ...
        createLink(
            "Twitter / X (@DerMittlereWeg)",
            "https://x.com/DerMittlereWeg",
            "🐦",
        );
        createLink(
            "Instagram (@anton0win)",
            "https://www.instagram.com/anton0win/",
            "📸",
        );
        createLink(
            "Email (awincommerce@gmail.com)",
            "mailto:awincommerce@gmail.com",
            "📧",
        );

        const finalWarn = el.createDiv();
        finalWarn.style.cssText =
            "margin-top: 25px; padding: 10px; text-align: center; border-top: 1px solid var(--background-modifier-border); color: #ff6400; font-weight: bold; font-size: 14px;";
        finalWarn.setText(
            "⚠️ This is an early version. Use at your own risk. Feedback is welcome!",
        );
    }
}
...
class PdfNotesPlugin extends Plugin {
    async onload() {
        ...
        if (!this.settings.toolIcons)
            this.settings.toolIcons = { ...DEFAULT_SETTINGS.toolIcons };
        ...
        if (
            ...
            !this.settings.enabledTools.includes("link")
        ) {
        ...
        this.addCommand({
            id: "open",
            name: "Open active PDF with PDF.notes",
            callback: () => {
                const f = this.app.workspace.getActiveFile();
                if (f?.extension === "pdf") this.openView(f);
                else new Notice("No active PDF.");
            },
        });
        this.registerEvent(
        ...
        this.addRibbonIcon("brush", "PDF.notes", () => {
            const f = this.app.workspace.getActiveFile();
            if (f?.extension === "pdf") this.openView(f);
            else new Notice("Open a PDF first.");
        });
        this.addRibbonIcon("file-plus-2", "PDF.notes (New blank file)", () =>
            this.createNewPdf(),
        );
    }
    async saveSettings() {
    ...
    async openView(tFile) {
        const leaf = this.app.workspace.getLeaf("tab");
        await leaf.setViewState({ type: VIEW_TYPE, active: true });
        const v = leaf.view;
        if (v instanceof PdfNotesView) {
            await v.setPdfFile(tFile);
        ...
        new CreatePdfModal(this.app, this, async (filename, folderPath) => {
            ...
            try {
                ...
                while (this.app.vault.getAbstractFileByPath(checkPath)) {
                    checkPath = normFolder
                        ? `${normFolder}/${baseName} ${i++}${ext}`
                        : `${baseName} ${i++}${ext}`;
                }
